...
const UserPage = () => {
  ...
  let loggeduser = localStorage.getItem("gymbuddyuser")
  ...
  useEffect(() => {
    if (userDetails?.height && userDetails?.weight) {
      ...
      const bmiCalculated = (userDetails.weight / (heightInMeters ** 2)).toFixed(1);
      setBmi(parseFloat(bmiCalculated));
    }
    ...
    <div>
      ...
      <div className="text-white shadow-lg rounded-lg overflow-hidden p-5 mt-5">
        ...
        <div className="flex justify-between mt-3">
          ...
          <div className="flex items-center">
            <FaFire className="text-white-500 text-2xl mr-2" />
            <span className="text-lg font-medium">BMI: {bmi ? `${bmi} kg/m²` : "Calculating..."}</span>
          </div>
          {/* <div className="flex items-center">
